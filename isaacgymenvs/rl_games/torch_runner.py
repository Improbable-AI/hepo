...
def _construct_multiplier(agent, args):
    from rl_games.algos_torch.lagrange_multiplier import LagrangianMultiplier
    lgrgn_mtpr = LagrangianMultiplier(**args)
    setattr(agent, "lgrgn_mtpr", lgrgn_mtpr)
...
class Runner:
    ...
    def run_train(self, args):
        ...
        if args['use_hepo']:
            from rl_games.algos_torch import hepo_continuous
            self.algo_factory.register_builder('a2c_continuous_hepo', lambda **kwargs : hepo_continuous.ContinuousHEPOAgent(**kwargs))
            algo_name = algo_name + '_hepo'
        agent = self.algo_factory.create(algo_name, base_name='run', params=self.params)
        _restore(agent, args)
