...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        self.save_freq = 0 # config.get('save_frequency', 0)
        self.save_best_after = np.inf # config.get('save_best_after', 100)
        ...
        self.game_success = copy.deepcopy(self.game_rewards)
        self.game_rewards_int = copy.deepcopy(self.game_rewards)
        ...
        if self.normalize_advantage and self.normalize_rms_advantage:
            ...
            if self.use_int:
                self.advantage_mean_std_int = GeneralizedMovingStats((1,), momentum=momentum).to(self.ppo_device)
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, c_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames):
        ...
        loss_record = {'losses/a_loss': torch_ext.mean_list(a_losses).item(),
                        'losses/c_loss': torch_ext.mean_list(c_losses).item(),
                        'losses/entropy': torch_ext.mean_list(entropies).item()}
        info_record = {'info/last_lr': last_lr * lr_mul,
                        'info/lr_mul': lr_mul,
        ...
        wandb.log(info_record, step=epoch_num)
        trigger_sync()
    ...
    def get_values(self, obs):
        ...
        with torch.no_grad():
            if self.has_central_value:
                ...
                value = result['values']
            if self.use_int:
                value_int = result['values_int']
            return {'ext': value, 'int': value_int}
    ...
    def init_tensors(self):
        ...
        if self.use_int:
            self.experience_buffer.tensor_dict['rewards_int'] = copy.deepcopy(\
                self.experience_buffer.tensor_dict['rewards'])
            self.experience_buffer.tensor_dict['values_int'] = copy.deepcopy(\
    ...
    def env_step(self, actions):
        ...
        if self.is_tensor_obses:
            ...
            return self.obs_to_tensors(obs), rewards_dict, success, dones.to(self.ppo_device), infos
        else:
            if self.value_size == 1:
    ...
    def compute_returns(self, fdones, last_values, mb_fdones, experience_buffer):
        mb_returns_int = None
        mb_values = experience_buffer.tensor_dict['values']
        mb_rewards = experience_buffer.tensor_dict['rewards']
        mb_advs = self.discount_values(fdones, last_values['ext'], mb_fdones, mb_values, mb_rewards)
        mb_returns = mb_advs + mb_values
        if self.use_int:
            mb_values_int = experience_buffer.tensor_dict['values_int']
            mb_rewards_int = experience_buffer.tensor_dict['rewards_int']
            mb_advs_int = self.discount_values(fdones, last_values['int'], mb_fdones, mb_values_int, mb_rewards_int)
            mb_returns_int = mb_advs_int + mb_values_int
        return mb_returns, mb_returns_int
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            self.obs, rewards, success, self.dones, infos = self.env_step(res_dict['actions'])
            ...
            if self.use_int:
                ...
                if self.use_int and self.value_bootstrap and 'time_outs' in infos:
                    shaped_rewards_int += self.gamma * res_dict['values_int'] * self.cast_obs(infos['time_outs']).unsqueeze(1).float()

                self.experience_buffer.update_data('rewards_int', n, shaped_rewards_int)
            ...
            self.game_rewards_int.update(self.current_rewards_int[env_done_indices])
            self.game_success.update(self.current_success[env_done_indices])        
        ...
        if self.use_int:
            batch_dict['returns_int'] = swap_and_flatten01(mb_returns_int)
    ...
    def play_steps_rnn(self):
        ...
        mb_returns, mb_returns_int = self.compute_returns(fdones, last_values, mb_fdones)
        batch_dict = self.experience_buffer.get_transformed_list(swap_and_flatten01, self.tensor_list)
        ...
        while True:
            ...
            if self.global_rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    mean_rewards_int = self.game_rewards_int.get_mean()
                    mean_success = self.game_success.get_mean()
                    ...
                    for i in range(self.value_size):
                        ...
                        reward_record.update({f'{rewards_name}/ext': mean_rewards[i], 
                                f'shaped_{rewards_name}/ext': mean_shaped_rewards[i]})
                    reward_record['episode_lengths/step'] = mean_lengths
...
class ContinuousA2CBase(A2CBase):
    ...
    def init_tensors(self):
        ...
        if self.use_int:
            self.update_list = self.update_list + ['values_int']
        self.tensor_list = self.update_list + ['obses', 'states', 'dones', 'rewards']
        if self.use_int:
            self.tensor_list = self.tensor_list + ['rewards_int']
        self.current_rewards_int = copy.deepcopy(self.current_rewards)
        self.current_success = copy.deepcopy(self.current_rewards)
    ...
    def prepare_dataset(self, batch_dict):
        ...
        if self.normalize_value:
            ...
            self.value_mean_std.eval()
            if returns_int is not None and values_int is not None:
                self.value_mean_std_int.train()
                values_int = self.value_mean_std_int(values_int)
                returns_int = self.value_mean_std_int(returns_int)
                self.value_mean_std_int.eval()
        ...
        if self.normalize_advantage:
            ...
            if advantages_int is not None:
                if self.is_rnn:
                    if self.normalize_rms_advantage:
                        advantages_int = self.advantage_mean_std_int(advantages_int, mask=rnn_masks)
                    else:
                        advantages_int = torch_ext.normalization_with_masks(advantages_int, rnn_masks)
                else:
                    if self.normalize_rms_advantage:
                        advantages_int = self.advantage_mean_std_int(advantages_int)
                    else:
                        advantages_int = (advantages_int - advantages_int.mean()) / (advantages_int.std() + 1e-8)

        if advantages_int is not None:
            predictions = {'mus': mus, 'sigmas': sigmas, 'actions': actions}
            advantages_mixed = self.lgrgn_mtpr.compute_advantages(advantages, advantages_int, 
                        rewards, predictions)
    ...
    def train(self):
        ...
        while True:          
            ...
            if self.global_rank == 0:
                ...
                if len(b_losses) > 0:
                    wandb.log({'losses/bounds_loss': torch_ext.mean_list(b_losses).item()}, step=epoch_num)
                    trigger_sync()

                if self.has_soft_aug:
                    wandb.log({'losses/aug_loss': np.mean(aug_losses)}, step=epoch_num)
                    trigger_sync()
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    for i in range(self.value_size):
                        rewards_name = 'rewards' if i == 0 else 'rewards{0}'.format(i)
                        reward_record.update({f'{rewards_name}/ppo_ext': mean_rewards[i],
                                            f'{rewards_name}/ppo_metric': mean_success[i], 
                                            f'shaped_{rewards_name}/ppo': mean_shaped_rewards[i]})
                        if self.use_int:
                            reward_record.update({f'{rewards_name}/ppo_int': mean_rewards_int[i]})
                    reward_record['episode_lengths/step'] = mean_lengths
                    wandb.log(reward_record, step=epoch_num)
                    trigger_sync()
                    if self.has_self_play_config:
