...
class BaseModel():
    ...
    def build(self, config):
        ...
        use_int = config.get('use_int', False)
        return self.Network(self.network_builder.build(self.model_class, **config), obs_shape=obs_shape,
            normalize_value=normalize_value, normalize_input=normalize_input, value_size=value_size, use_int=use_int)

class BaseModelNetwork(nn.Module):
    def __init__(self, obs_shape, normalize_value, normalize_input, value_size, use_int=False):
        ...
        if normalize_value:
            ...
            if self.use_int:
                self.value_mean_std_int = RunningMeanStd((self.value_size,))
        if normalize_input:
    ...
    def denorm_value(self, value, use_int=False):
        with torch.no_grad():
            if use_int:
                return self.value_mean_std_int(value, denorm=True) if self.normalize_value else value
            else:
...
class ModelA2C(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            logits, value, value_int, states = self.a2c_network(input_dict)
            ...
            if is_train:
                ...
                if value_int is not None:
                    result['values_int'] = self.denorm_value(value_int, 
                                                             use_int=True)
                return  result
...
class ModelA2CContinuous(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            prev_actions = input_dict.get('prev_actions', None)
            input_dict['obs'] = self.norm_obs(input_dict['obs'])
            mu, sigma, value, value_int, states = self.a2c_network(input_dict)
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            prev_actions = input_dict.get('prev_actions', None)
            input_dict['obs'] = self.norm_obs(input_dict['obs'])
            mu, logstd, value, value_int, states = self.a2c_network(input_dict)
