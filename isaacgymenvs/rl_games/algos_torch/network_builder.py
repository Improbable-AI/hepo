...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            self.use_int = kwargs.pop('use_int', False)
            ...
            if self.use_int:
                self.critic_cnn_int = nn.Sequential()
            if self.has_cnn:
                ...
                if self.separate:
                    ...
                    if self.use_int:
                        self.critic_cnn_int = self._build_conv( **cnn_args)
            ...
            if self.has_rnn:
                ...
                if self.separate:
                    self.actor_rnn = self._build_rnn(self.rnn_name, rnn_in_size, self.rnn_units, self.rnn_layers)
                    self.critic_rnn = self._build_rnn(self.rnn_name, rnn_in_size, self.rnn_units, self.rnn_layers)
                    if self.rnn_ln:
                        self.actor_layer_norm = torch.nn.LayerNorm(self.rnn_units)
                        self.critic_layer_norm = torch.nn.LayerNorm(self.rnn_units)
                    if self.use_int:
                        self.critic_rnn_int = self._build_rnn(self.rnn_name, rnn_in_size, self.rnn_units, self.rnn_layers)
                        if self.rnn_ln:
                            self.critic_layer_norm_int = torch.nn.LayerNorm(self.rnn_units)
                else:
            ...
            if self.separate:
                ...
                if self.use_int:
                    self.critic_mlp_int = self._build_mlp(**mlp_args)    

            self.value = self._build_value_layer(out_size, self.value_size)
            self.value_act = self.activations_factory.create(self.value_activation)
            if self.use_int:
                self.value_int = self._build_value_layer(out_size, self.value_size)
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                if self.use_int:
                    c_out_int = self.critic_cnn_int(obs[:])
                    c_out_int = c_out_int.contiguous().view(c_out_int.size(0), -1)                    
                ...
                if self.has_rnn:
                    ...
                    c_cnn_out = c_out
                    if self.use_int:
                        c_cnn_out_int = c_out_int
                    if not self.is_rnn_before_mlp:
                        a_out = self.actor_mlp(a_cnn_out)
                        c_out = self.critic_mlp(c_cnn_out)
                        if self.use_int:
                            c_out_int = self.critic_mlp_int(c_cnn_out_int)
                    ...
                    if self.use_int:
                        c_out_int = c_out_int.reshape(num_seqs, seq_length, -1)
                    ...
                    if len(states) == 2:
                        ...
                        a_states = states[:2]
                        c_states = states[2:]                        
                    a_out, a_states = self.actor_rnn(a_out, a_states, dones, bptt_len)
                    c_out, c_states = self.critic_rnn(c_out, c_states, dones, bptt_len)
                    if self.use_int:
                        c_out_int, _ = self.critic_rnn(c_out_int, c_states, dones, bptt_len)
                    ...
                    if self.use_int:
                        c_out_int = c_out_int.transpose(0,1)
                        c_out_int = c_out_int.contiguous().reshape(c_out_int.size()[0] * \
                                                                   c_out_int.size()[1], -1)
                        
                    if self.rnn_ln:
                        a_out = self.actor_layer_norm(a_out)
                        c_out = self.critic_layer_norm(c_out)
                        if self.use_int:
                            c_out_int = self.c_layer_norm_int(c_out_int)
                    ...
                    if self.rnn_concat_output:
                        ...
                        c_out = torch.cat([c_out, c_cnn_out], dim=1)
                        if self.use_int:
                            c_out_int = torch.cat([c_out_int, c_cnn_out_int], dim=1)

                    if self.is_rnn_before_mlp:
                        a_out = self.actor_mlp(a_out)
                        c_out = self.critic_mlp(c_out)
                        if self.use_int:
                            c_out_int = self.critic_mlp_int(c_out_int)
                else:
                    a_out = self.actor_mlp(a_out)
                    c_out = self.critic_mlp(c_out)
                    if self.use_int:
                ...
                value_int = None
                if self.use_int:
                    value_int = self.value_act(self.value_int(c_out_int))

                if self.is_discrete:
                    logits = self.logits(a_out)
                    return logits, value, value_int, states
                ...
                if self.is_continuous:
                    ...
                    return mu, sigma, value, value_int, states
            else:
                out = obs
                ...
                value_int = None
                if self.use_int:
                    value_int = self.value_act(self.value_int(out))
                ...
                if self.is_continuous:
                    ...
                    return mu, mu*0 + sigma, value, value_int, states
