...
class ShadowHandReOrientation(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        """
        self.rew_buf[:], goal_reach, self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.consecutive_successes[:] = compute_hand_reward(
            self.rew_buf, self.reset_buf, self.reset_goal_buf, self.progress_buf, self.successes, self.consecutive_successes,
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.object_another_pos, self.object_another_rot, self.goal_another_pos, self.goal_another_rot,
            self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
            self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
        ...
        self.success_buf[:] = goal_reach[:] / (self.max_episode_length - 1)
...
def compute_hand_reward(
    ...
    goal_reach = 0.5 * (torch.where(torch.abs(rot_dist) <= 0.1, torch.ones_like(reset_goal_buf), torch.zeros_like(reset_goal_buf)) \
            + torch.where(torch.abs(rot_another_dist) <= 0.1, torch.ones_like(reset_goal_buf), torch.zeros_like(reset_goal_buf)))
    return reward, goal_reach, resets, goal_resets, progress_buf, successes, cons_successes