...
class ShadowHandReOrientation_orig(VecTask):
    """
...
def compute_hand_reward(
    ...
    # goal_resets = torch.where(torch.abs(rot_dist) < 0.1, 
    #     torch.ones_like(reset_goal_buf), reset_goal_buf)
    # goal_resets = torch.where(torch.abs(rot_another_dist) < 0.1, 
    #     torch.ones_like(reset_goal_buf), reset_goal_buf)
    goal_resets = torch.where(torch.abs(rot_dist) < 0.1, 
            torch.where(torch.abs(rot_another_dist) < 0.1, 
